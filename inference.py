...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            """if noisyDistance is None:
                allPossible[p] = util.manhattanDistance(jposition, pacmanPosition)"""
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for newPos, prob in newPosDist.items():
            print(newPos)
            self.setGhostPosition(gameState, newPos)
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        if noisyDistance is None:               #jail case
            ...
            for p in self.legalPositions:
                Distance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[Distance]*beliefs[p]
            """allPossible.normalize()
            self.beliefs = allPossible"""
            temp = []
            if allPossible.totalCount() == 0:         #handdel weight = 0
                self.initializeUniformly(gameState)
            else:
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for pos in self.particles:
            temp = 1
            for gos in range(self.numGhosts):          #jail case where noisydistances are none
                if noisyDistances[gos] is None:
                    pos = self.getParticleWithGhostInJail(pos, gos)                   
                else:
                    Distance = util.manhattanDistance(pos[gos],pacmanPosition)
                    temp *= emissionModels[gos][Distance]
            allPossible[pos] += temp

        if allPossible.totalCount() == 0:            #0 weight case 
            self.initializeParticles()
        else:
            temp = []
            for i in range(0, self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            prevGhostPositions = list(oldParticle) #put the previous ghost position into a list
            "*** YOUR CODE HERE ***"
            temp = []
            for i in range(self.numGhosts):
